...
G_DEFINE_TYPE_WITH_CODE (AppCamera, app_camera, G_TYPE_OBJECT,
                         G_ADD_PRIVATE (AppCamera))
...
static void app_camera_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
    ...
    switch (property_id) {
        ...
        case PROP_WIDTH:       g_value_set_uint(value, priv->width); break;
        case PROP_HEIGHT:      g_value_set_uint(value, priv->height); break;
        case PROP_FPS:         g_value_set_uint(value, priv->fps); break;
        case PROP_STATUS:      g_value_set_int(value, priv->status); break;
        case PROP_PAN:         g_value_set_double(value, priv->pan); break;
        case PROP_TILT:        g_value_set_double(value, priv->tilt); break;
        case PROP_ZOOM:        g_value_set_double(value, priv->zoom); break;
        case PROP_IS_ENABLED:  g_value_set_boolean(value, priv->is_enabled); break;
        case PROP_IS_RECORDING: g_value_set_boolean(value, priv->is_recording); break;
...
static void app_camera_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
    AppCamera *self = APP_CAMERA(object);
    AppCameraPrivate *priv = app_camera_get_instance_private(self);
    ...
    switch (property_id) {
        ...
        case PROP_NAME: 
            ...
            break;
        case PROP_WIDTH: 
            priv->width = MIN(g_value_get_uint(value), 4000); 
            break;
        case PROP_HEIGHT: 
            priv->height = MIN(g_value_get_uint(value), 2200); 
            break;
        case PROP_FPS: 
            priv->fps = CLAMP(g_value_get_uint(value), 1, 120); 
            break;
        case PROP_STATUS: 
            priv->status = g_value_get_int(value); 
            break;
        case PROP_PAN: 
            priv->pan = CLAMP(g_value_get_double(value), -180.0, 180.0); 
            break;
        case PROP_TILT: 
            priv->tilt = CLAMP(g_value_get_double(value), -90.0, 90.0); 
            break;
        case PROP_ZOOM: 
            priv->zoom = CLAMP(g_value_get_double(value), 0.0, 5.0); 
            break;
...
static void app_camera_class_init(AppCameraClass *klass) {  // điền các giá trị khởi tạo vào bảng cũng như các hàm tương ứng với các con trỏ hàm
    ...
    obj_properties[PROP_STATUS] = g_param_spec_int (  /* Đổi từ enum sang int */
    "status",
    "Status",
    "Camera Status",
    APP_CAMERA_STATUS_DISCONNECTED, /* Minimum */
    APP_CAMERA_STATUS_ERROR,        /* Maximum */
